...
function Cart() {
    // استخدمنا الـ Lazy Initializer فقط لضمان قراءة البيانات فوراً
    const [cart, setCart] = useState<CartData | null>(() => {
        const rawCart = localStorage.getItem("cart");
        return rawCart ? JSON.parse(rawCart) : null;
    });

    // هذا الـ Effect مفيد فقط إذا كنت تريد تحديث العربة إذا تغيرت في Tab أخرى
    useEffect(() => {
